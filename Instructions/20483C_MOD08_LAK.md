...
2. Initialize database:
    -  In the **Apps** list, click **File Explorer**.
       In File Explorer, navigate to the **[Repository Root]\\Mod08\\Labfiles\\Databases** folder,
...
then click **Open**.
4.  In Solution Explorer, right-click the **GradesPrototype** solution, point to
    **Add**, and then click **New Project**.
5.  In the **Add New Project** dialog box, on the left pane expand **Visual
    C\#**, and then click **Web**.
6.  In the templates list, click **ASP.NET Web Application**, then follow the steps:
    -  In the **Framework** combobox, select **.NET Framework 4.7**. 
    -  In the **Name** box, type **Grades.Web**.
    -  Click **OK**.
8.  In the **New ASP.NET Web Application - Grades.Web** dialog box select **Empty**, and then click **OK**.  
9.  In Solution Explorer, right-click the **Grades.Web** project, and then click
    **Properties**.
10.  On the **Web** tab, in the **Start Action** section, click **Don’t open a page. Wait for a request from an external application**.
11.  In the **Servers** section, ensure that **IIS Express** is
    selected.
12.  In the **Project Url** box, type **http://localhost:1650/**, and then click
    **Create Virtual Directory**.
13.  In the **Microsoft Visual Studio** dialog box, click **OK**.
14. In Solution Explorer, right-click the **GradesPrototype** solution, and then
    click **Set StartUp Projects**.
15. In the **Solution 'GradesPrototype' Property Pages** dialog box, click
    **Multiple startup projects**.
16. In the **Action** column for **Grades.Web** and **GradesPrototype**, click
...
1.  In Solution Explorer, right-click the **GradesPrototype** solution, and then
    click **Restore NuGet Packages**.
2.  In Solution Explorer, right-click the **Grades.Web** project, point to
    **Add**, and then click **New Folder**.
3.  Delete the existing folder name, type **Services**, and then press Enter.
4.  Right-click **Services**, point to **Add**, and then click **New Item**.
5.  In the templates list, click **WCF Data Service**.
6.  In the **Name** box, type **GradesWebDataService**, and then click **Add**.
7.  In the **Grades.Web** project, right-click on **Referencs**, and then click **Add Reference**.
8.  In the **Reference Manager – Grades.Web** dialog box do the following steps:
    - Click on **Projects**,and then select **Grades.DataModel**.
    - Click **Browse**.
    - In the **Select the files to reference** dialog box, browse to the
    **[Repository Root]\\Mod08\\Labfiles\\Starter\\Exercise 1\\packages\\EntityFramework.5.0.0\\lib\\net45** folder, click
    **EntityFramework.dll**, and click **Add**.
    - Click on **Assemblies**, locate **System.Data.Services** and select the checkbox.
    - Click **OK**.
12.	In Solution Explorer, expand the **GradesPrototype** project, and then double-click **App.config**. 
13.	In the code editor, copy the following XML to the clipboard:
    ```xml
    <connectionStrings>
    ...
    ```
14.	In Solution Explorer, in the **Grades.Web** project, double-click **Web.config**.
15.	Click at the end of the opening \<configuration\> element, press Enter, and then paste the contents of the clipboard.

#### Task 3: Specify the GradesDBEntities data context for the data service.

1. In Solution Explorer, in **Grades.Web** project, right-click on **properties**, then select 
